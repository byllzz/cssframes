...
export default function SharePanel({ animation, isOpen, onClose }) {
  ...
  // Helper to create a URL-friendly name (Slug)
  // e.g. "Hyper Glow Pulse" -> "hyper-glow-pulse"
  ...
  const slug = createSlug(animation.title) || 'animation';
  // Final URL includes the ID and the clean name
  const shareUrl = `https://cssframes.vercel.app/preview/${animation.id}/${slug}`;
  ...
  const shareToTwitter = () => {
    const text = encodeURIComponent(`Check out this "${animation.title}" animation I found on CSSFrames! 🚀`);
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button
              ...
              className="flex items-center cursor-pointer justify-center gap-3 p-4 bg-zinc-900 border border-white/5 rounded-[5px] hover:border-white/20 transition-all group"
            >
...
function TwitterSvg({ height = 20, className }) {
  return (
