...
export default function Navbar({ activeNavigation, setActiveNavigation, handleStartCreating }) {
  ...
  return (
    ...
    <nav className="bg-[#050505] w-full font-outfit relative z-50 px-4 lg:px-4">
      <div className="max-w-full mx-auto flex items-center justify-between h-16">
            ...
            {navLinks.map(item => (
              <button
                ...
                className={`text-[15px] tracking-wider font-medium px-4 py-[7.5px] rounded-[5px] cursor-pointer transition-all ${
                  activeNavigation === item ? ' bg-[#161616] text-white' : 'text-white  hover:bg-[#161616]'
                }`}
              >
                {item}
              </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="lg:hidden text-zinc-400 hover:text-white bg-zinc-900 p-2 rounded-[5px] border border-zinc-800 transition-all"
          >
      ...
      {/* mob menu overlay */}
      {isOpen && (
        <div className="lg:hidden absolute top-[65px] left-0 right-0 bg-[#050505] border-b border-zinc-800 p-5 animate-in fade-in slide-in-from-top-4 z-[200] shadow-2xl">
