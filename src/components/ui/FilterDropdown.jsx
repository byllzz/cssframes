...
export default function FilterDropdown({ previewOptions, setPreviewType, previewType }) {
  ...
  // get current label
const currentOption = previewOptions.find(opt => opt.id === previewType);
const currentLabel = currentOption?.label ?? 'Box';

  return (
    <div className="relative inline-block font-outfit z-[9999]" ref={dropdownRef}>
      {/* trigger */}
      <button
        onClick={e => {
          e.stopPropagation();
      ...
      >
        ...
        <span className="text-[13px] font-medium text-zinc-400 border-r border-zinc-800 pr-3 mr-1 inline">
          Preview Object
        </span>

        <div className="flex items-center gap-2">
          <span className="text-[14px] font-medium text-zinc-100 capitalize">{currentLabel}</span>
          <ChevronDown
            size={16}
      ...
      {/* menu overlay */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-48 bg-[#000] rounded-[8px] z-[9999] overflow-hidden p-1.5">
            ...
            {previewOptions.map(option => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <span className={isActive ? 'text-black' : 'text-zinc-500'}>{option.icon}</span>
                  {option.label}
