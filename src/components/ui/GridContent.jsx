...
const containerVariants = {
  ...
  show: {
    transition: {
      staggerChildren: 0.06, // delay between cards
    },
  },
};
...
export default function GridContent({
  ...
  const [gridPattern, setGridPattern] = useState(() => {
    return localStorage.getItem('cssframes_grid_pattern') || 'grid';
  });
  const cardsPerPage = 20; // Multiples of 2, 3, and 4 work best for responsive grids
  // Save gridPattern to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem('cssframes_grid_pattern', gridPattern);
  }, [gridPattern]); // Ref
  const gridTopRef = useRef(null); // Filter Logic
      ...
      {' '}
       ...
       {/* Main Grid Content */}{/* Main Grid Content */}
        <motion.main
          ...
          initial="hidden"
          animate="show" // REMOVED previewType from here
          key={currentPage + selectedCategory + searchQuery}
          className={`px-3 md:px-2 transition-all duration-500 ${
            gridPattern === 'grid'
              ? 'grid grid-cols-2 justify-center sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3'
        ...
        {' '}
          {currentCards.length > 0 ? (
            currentCards.map(anim => (
              <motion.div key={anim.id} variants={cardVariants}>
            {' '}
                <AnimationCard
                  animation={anim}
