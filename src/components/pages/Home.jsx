...
const KEYFRAMES = `
  ...
  .discord-card:hover { border-color: rgba(88,101,242,0.45); transform: translateY(-2px); }
  .discord-card::before {
  ...
  .figma-card {
    position: relative; overflow: hidden;
    border-radius: 16px;
    background: linear-gradient(135deg, #0f0d14, #0a0a0f);
    border: 1px solid rgba(167,139,250,0.15);
    transition: border-color .3s ease, transform .3s ease;
  }
  .figma-card:hover { border-color: rgba(167,139,250,0.35); transform: translateY(-2px); }

  .cat-card-v2 {
    position: relative; overflow: hidden; cursor: pointer;
    border-radius: 16px; background: #0a0a0f;
  ...
  .cat-card-v2::after {
    ...
    background: linear-gradient(135deg, var(--accent-a), transparent 60%);
  ...
  .cat-card-v2:hover .shimmer-line { animation: card-shimmer .7s ease forwards; }
  .cat-count { font-variant-numeric: tabular-nums; transition: color .3s ease; }
  .cat-card-v2:hover .cat-count { animation: count-glow 1.5s ease infinite; }
  .arrow-icon { opacity: 0; transform: translateX(-6px); transition: opacity .25s ease, transform .25s ease; }
  .cat-card-v2:hover .arrow-icon { opacity: 1; transform: translateX(0); }
...
const ROW_WIDTHS = [
  [220, 280, 200, 260, 240, 300, 220, 280, 200, 260, 240, 300],
  [260, 200, 300, 220, 280, 240, 260, 200, 300, 220, 280, 240],
  [240, 300, 220, 280, 200, 260, 240, 300, 220, 280, 200, 260],
];
...
export default function Home({
  ...
  const totalCommunity = animations.filter((a) => a.isCommunity).length;
      ...
      <div className="font-grotesk relative min-h-screen overflow-x-hidden text-[#f0eeff] bg-[#050505]">
        ...
        <div className="relative z-[5] bottom-28 overflow-hidden bg-black">
          {[0, 1, 2].map((row) => (
            <div
              key={row}
              className={`flex gap-3 ${row < 2 ? "mb-3" : ""}`}
